module Lorenz
    ...
    integer,parameter :: MaxT=2500000,T_trans=2400000,N=3,M=100,number_cycle=200
    real :: x(M,N),epsilon,R,omega(M)
    integer :: number_cycle_state(7),gradient_number_omega
...
!            write(5,*) x(i,1),x(i,2),x(i,3)
    ...
    subroutine Order_Parameter_Omega(sequence)
        implicit none
        integer,parameter :: cycle_Number=10
    ...
    subroutine Order_Parameter_Multiple_states(U_max,U_min,U_max1,U_min1)
        implicit none
        integer :: i,j,total_Rest,Traveling_number,state(7)
        real :: U_max(M),U_min(M),U_max1(M),U_min1(M)
        total_Rest=0
        Traveling_number=0
        state=0
        do i=1,M,1
...
!        write(60,*) epsilon,total_Rest/real(M)
        ...
        write(70,*) epsilon,state(1),state(2),state(3),state(4),state(5),state(6),state(7)
        return
...
program main
    ...
    real :: U_max(M),U_min(M),sequence1(M,MaxT-T_trans),U_max1(M),U_min1(M)
    ...
    do epsilon=0.5, 50, 0.5

        number_cycle_state=0
        do j=1,number_cycle,1
            write(*,*) "epsilon=",epsilon,"counter=",j
            U_max=-10000.0
            U_min=10000.0
            U_max1=-10000.0
            U_min1=10000.0
...
!                    end do
            ...
            call Order_Parameter_Omega(sequence1)

            call Order_Parameter_R(sequence1)
            !write(80,*) epsilon,R
        ...
        write(90,*) epsilon,number_cycle_state(1)/real(number_cycle),number_cycle_state(2)/real(number_cycle),&
                ...
                number_cycle_state(7)/real(number_cycle)
    end do
    close(10)
    close(20)
    close(30)
